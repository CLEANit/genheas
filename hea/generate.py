...
from ase.build import fcc111, fcc110, fcc100, bcc111, bcc110, bcc100
...
def generate_structure(my_model, output_name, element_pool, conc,
                       cryst_structure, cell_size, cell_param, constraints=True, cubik=False, direction=None,
    ...
    max_diff_elements = AlloyGen.get_max_diff_elements(
        element_pool, conc, nb_atom)

    logger.info(f'number of atom : {nb_atom}')
    ...
    structureX = AlloyGen.gen_random_structure(
        cryst_structure, cell_size, max_diff_elements, lattice_param=cell_param, name=element_pool[0], cubik=cubik,
        surface=direction)

    structureX = AseAtomsAdaptor.get_structure(structureX)

    configuration = AlloyGen.generate_configuration(
        structureX, element_pool, cutof, my_model, device=device, max_diff_element=max_diff_elements,
        constrained=constraints, verbose=verbose
    )
    ...
    shell1_fitness_AB = GaNn.get_shell1_fitness_AB(
        CN1_list, conc, nn_in_shell1)
    ...
    print('max_diff_element_fitness: {:6.2f}'.format(
        sum(max_diff_element_fitness.values())))
    print('*' * 60)

    logger.info(f' <.cif> files have been saved in [{output_name}]')
    logger.info('Total time for the generation h:m:s ::  {}'.format(
...
if __name__ == '__main__':
    ...
    for i in range(nb_structures):
        ...
        shutil.move(
            os.path.join(
                output, 'structure.cif'), os.path.join(
                output, f'structure_{i:03d}.cif'))
        # shutil.move(
        #     os.path.join(
